/*********************************************************************
 * trackFeatures.c   -- Batched OpenACC for gradient sum only
 *********************************************************************/
...
/* --------------------------------------------------------------------
   Interpolation (serial, used by intensity difference)
   -------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------
   Helpers for window indexing
   -------------------------------------------------------------------- */
static inline int _buf_index_local(int i, int j, int hw, int hh, int width)
{
    return (j + hh) * width + (i + hw);
}

static inline int _buf_index_batched(int feat, int i, int j, int hw, int hh, int width, int feat_stride)
{
    return feat * feat_stride + _buf_index_local(i, j, hw, hh, width);
}

/* --------------------------------------------------------------------
   Gradient sum batch (optimized with OpenACC)
   -------------------------------------------------------------------- */
static void _computeGradientSum_batch(
    ...
    _FloatWindow gradx_out,
    _FloatWindow grady_out)
{
    int hw = width / 2;
    int hh = height / 2;
    int total_per_feat = width * height;

    float* data_x1 = gradx1->data;
    float* data_y1 = grady1->data;
    float* data_x2 = gradx2->data;
    float* data_y2 = grady2->data;

    int ncols_x1 = gradx1->ncols, nrows_x1 = gradx1->nrows;
    int ncols_y1 = grady1->ncols, nrows_y1 = grady1->nrows;
    int ncols_x2 = gradx2->ncols, nrows_x2 = gradx2->nrows;
    int ncols_y2 = grady2->ncols, nrows_y2 = grady2->nrows;

    int size_x1 = ncols_x1 * nrows_x1;
    int size_y1 = ncols_y1 * nrows_y1;
    int size_x2 = ncols_x2 * nrows_x2;
    int size_y2 = ncols_y2 * nrows_y2;

#pragma acc parallel loop collapse(3) \
    copyin(data_x1[0:size_x1], data_y1[0:size_y1], data_x2[0:size_x2], data_y2[0:size_y2], \
           x1_arr[0:nFeatures], y1_arr[0:nFeatures], x2_arr[0:nFeatures], y2_arr[0:nFeatures]) \
    copyout(gradx_out[0:(nFeatures*total_per_feat)], grady_out[0:(nFeatures*total_per_feat)])
    for (int feat = 0; feat < nFeatures; ++feat) {
        for (int j = -hh; j <= hh; ++j) {
            for (int i = -hw; i <= hw; ++i) {
                float sx1 = x1_arr[feat] + (float)i;
                float sy1 = y1_arr[feat] + (float)j;
                float sx2 = x2_arr[feat] + (float)i;
                float sy2 = y2_arr[feat] + (float)j;

                /* gradx1 */
                int xt1 = (int)floorf(sx1);
                int yt1 = (int)floorf(sy1);
                float ax1 = sx1 - xt1;
                float ay1 = sy1 - yt1;
                float* p_x1 = data_x1 + yt1 * ncols_x1 + xt1;
                float gx1 = (1 - ax1) * (1 - ay1) * p_x1[0] + ax1 * (1 - ay1) * p_x1[1] +
                    (1 - ax1) * ay1 * p_x1[ncols_x1] + ax1 * ay1 * p_x1[ncols_x1 + 1];

                /* gradx2 */
                int xt2 = (int)floorf(sx2);
                int yt2 = (int)floorf(sy2);
                float ax2 = sx2 - xt2;
                float ay2 = sy2 - yt2;
                float* p_x2 = data_x2 + yt2 * ncols_x2 + xt2;
                float gx2 = (1 - ax2) * (1 - ay2) * p_x2[0] + ax2 * (1 - ay2) * p_x2[1] +
                    (1 - ax2) * ay2 * p_x2[ncols_x2] + ax2 * ay2 * p_x2[ncols_x2 + 1];

                /* grady1 */
                float* p_y1 = data_y1 + yt1 * ncols_y1 + xt1;
                float gy1 = (1 - ax1) * (1 - ay1) * p_y1[0] + ax1 * (1 - ay1) * p_y1[1] +
                    (1 - ax1) * ay1 * p_y1[ncols_y1] + ax1 * ay1 * p_y1[ncols_y1 + 1];

                /* grady2 */
                float* p_y2 = data_y2 + yt2 * ncols_y2 + xt2;
                float gy2 = (1 - ax2) * (1 - ay2) * p_y2[0] + ax2 * (1 - ay2) * p_y2[1] +
                    (1 - ax2) * ay2 * p_y2[ncols_y2] + ax2 * ay2 * p_y2[ncols_y2 + 1];

                int idx = _buf_index_batched(feat, i, j, hw, hh, width, total_per_feat);
...
/* --------------------------------------------------------------------
   Single-feature wrapper for gradient sum
   -------------------------------------------------------------------- */
static void _computeGradientSum_single(
    _KLT_FloatImage gradx1,
    _KLT_FloatImage grady1,
    _KLT_FloatImage gradx2,
    _KLT_FloatImage grady2,
    ...
    _FloatWindow grady)
{
    ...
    float x1_arr[1] = { x1 }, y1_arr[1] = { y1 }, x2_arr[1] = { x2 }, y2_arr[1] = { y2 };

    _FloatWindow batched_x = (_FloatWindow)malloc(sizeof(float) * total);
    _FloatWindow batched_y = (_FloatWindow)malloc(sizeof(float) * total);

    if (!batched_x || !batched_y) {
        /* fallback serial */
        int hw = width / 2, hh = height / 2;
        for (int j = -hh;j <= hh;j++)
            for (int i = -hw;i <= hw;i++) {
                float gx1 = _interpolate(x1 + i, y1 + j, gradx1);
                float gx2 = _interpolate(x2 + i, y2 + j, gradx2);
                gradx[_buf_index_local(i, j, hw, hh, width)] = gx1 + gx2;
                float gy1 = _interpolate(x1 + i, y1 + j, grady1);
                float gy2 = _interpolate(x2 + i, y2 + j, grady2);
                grady[_buf_index_local(i, j, hw, hh, width)] = gy1 + gy2;
            }
        if (batched_x) free(batched_x);
        if (batched_y) free(batched_y);
        return;
    }

    _computeGradientSum_batch(gradx1, grady1, gradx2, grady2,
    ...
    free(batched_x); free(batched_y);
}

/* --------------------------------------------------------------------
   Serial intensity difference (no OpenACC)
   -------------------------------------------------------------------- */
static void _computeIntensityDifference_single(
    ...
    _FloatWindow imgdiff)
{
    ...
    for (int j = -hh;j <= hh;j++)
        for (int i = -hw;i <= hw;i++)
            imgdiff[_buf_index_local(i, j, hw, hh, width)] =
            _interpolate(x1 + i, y1 + j, img1) - _interpolate(x2 + i, y2 + j, img2);
}

/* --------------------------------------------------------------------
   Compatibility wrappers (original KLT function names)
   -------------------------------------------------------------------- */
static void _computeIntensityDifference(
    ...
    _FloatWindow imgdiff)
{
    _computeIntensityDifference_single(img1, img2, x1, y1, x2, y2, width, height, imgdiff);
}
...
static void _computeGradientSum(
    ...
    _FloatWindow grady)
{
    _computeGradientSum_single(gradx1, grady1, gradx2, grady2,
